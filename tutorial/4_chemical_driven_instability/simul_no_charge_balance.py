...
class multicomponent_transport_problem_uzawa(multicomponent_transport_problem):
    ...
    def solve(self, dt_num, dt_end):
        ...
        while(current_time < dt_end):
            ...
            self.xdmf_obj.write_checkpoint(self.charge_func, self.charge_func.name(), current_time, append=True)
            #self.xdmf_obj.write_checkpoint(self.adv, self.adv.name(), current_time, append=True)
            self.xdmf_obj.write_checkpoint(self.p0, self.p0.name(), current_time, append=True)
            self.xdmf_obj.write_checkpoint(self.pH_func, self.pH_func.name(), current_time, append=True)
            self.xdmf_obj.write_checkpoint(self.rho, self.rho.name(), current_time, append=True)

            for j in range(self.num_transport_components):
                #self.X_list_old[j].assign(self.X_list[j])
                #self.xdmf_obj.write(self.X_list_old[j], current_time)
                self.xdmf_obj.write_checkpoint(self.X_list_old[j], self.X_list_old[j].name(), current_time, append=True)
...
diffusivity = [5.0e-3, 5.0e-3, 5.0e-3, 5.0e-3]
...
problem.set_transport_equations(charge_balance=False)
