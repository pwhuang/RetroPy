...
class RotatingCone(EllipticTransportBenchmark):
    ...
    def solution_expr(x):
        ...
        return np.where(((x[0]-x0)/sigma)**2 + ((x[1]-x1)/sigma)**2 < 0.1,
    ...
    def get_center_of_mass(self):
        comm = self.mesh.comm
        center_x = assemble_scalar(form(self.fluid_comp_sub[0]*self.cell_coord.sub(0)*self.dx))
        center_y = assemble_scalar(form(self.fluid_comp_sub[0]*self.cell_coord.sub(1)*self.dx))
