...
from retropy.problem import TracerTransportProblem, DarcyFlowMixedPoisson
...
class MeshFactory(MarkedRectangleMesh):
    ...
    def locate_and_mark_boundaries(self, boundary_eps=1e-8):
        ...
        bottom_outer_marker = lambda x: np.logical_and(
            np.isclose(x[1], self.ymin, atol=boundary_eps),
            np.logical_or(x[0] > 3.0 - boundary_eps, x[0] < 1.0 + boundary_eps),
        )
...
class FlowManager(DarcyFlowMixedPoisson):
    ...
    def setup_flow_solver(self):
        ...
        self.set_flow_solver_params(options_v, options_p)
...
class TransportManager(TracerTransportProblem, DG0Kernel, PETScSolver):
    ...
    def define_problem(self, Rayleigh_number=400.0):
        ...
        self.mark_component_boundary(
            {"Temp": [self.marker_dict["top"], self.marker_dict["bottom_inner"]]}
        )

        self.temp_bc = [
            Constant(self.mesh, ScalarType(0.0)),
    ...
    def add_physics_to_form(self, u, kappa, f_id=0):
        ...
        for component in self.component_dict.keys():
            ...
            self.add_component_diffusion_bc(
                component,
                Constant(self.mesh, ScalarType(1e2)),
                self.temp_bc,
                kappa,
                f_id,
            )
    ...
    def setup_transport_solver(self):
        self.advection_velocity = as_vector([self.Ra * self.fluid_velocity])
...
class ElderProblem(TransportManager, FlowManager, OutputManager):
    ...
    def solve(self, dt_val=1.0, timesteps=1):
        ...
        for i in range(timesteps):
            ...
            self.solve_flow(target_residual=5e-9, max_steps=10)
        ...
        if self.mesh.comm.Get_rank() == 0:
            np.save(
                self.output_file_name + "_time",
                np.array(saved_times),
                allow_pickle=False,
            )


problem = ElderProblem(nx=640, ny=160, mesh_type="quadrilateral", mesh_shape="crossed")
problem.solve(dt_val=5e-5, timesteps=600)
