...
from retropy.manager import FlowManager
...
class BoundaryEquilibriumProblem(MassBalanceBase, ReactionManager):
    ...
    def set_chem_system(self, database="supcrt07"):
        ...
        self.chem_system = rkt.ChemicalSystem(
            db, self.aqueous_phase, self.gaseous_phase
        )

    def set_activity_models(self):
        self.aqueous_phase.set(
            rkt.chain(rkt.ActivityModelHKF(), rkt.ActivityModelDrummond("CO2"))
        )
...
class ReactiveTransportManager(ReactiveTransportManager):
    def setup_auxiliary_reaction_solver(self):
        ...
        self.aux_equi_problem = BoundaryEquilibriumProblem(
            self.component_str, self.solvent_name
        )
    ...
    def mark_inflow_boundary_cells(self):
        ...
        cell_marker = assemble_vector(
            form(w * one * self.ds(self.marker_dict["top"]))
        ).array[:]
    ...
    def _solve_chem_equi_over_dofs(self, pressure, fluid_comp):
        ...
        for i in self.boundary_cell_idx:
            self.aux_equi_problem._set_species_amount(
                list(fluid_comp[i])
            ...
            self.molar_density_temp[i] = self.aux_equi_problem._get_species_amounts()[
                :-1
            ]


class FlowManager(FlowManager, DarcyFlowUzawa):
    def set_flow_fe_space(self):
        self.set_pressure_fe_space("DG", 0)
        self.set_velocity_fe_space("RTCF", 1)


class Problem(ReactiveTransportManager, FlowManager, OutputManager, TransientNLSolver):
    ...
    def set_component_properties(self):
        self.set_molar_mass(
            [6.941, 1.00794, 17.00734, 44.0095, 60.0089, 61.01684]
        )  # g/mol
