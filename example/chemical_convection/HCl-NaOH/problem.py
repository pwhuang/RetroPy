...
class FlowManager(FlowManager, DarcyFlowMixedPoisson):
...
class Problem(ReactiveTransportManager, FlowManager, OutputManager, TransientNLSolver):
    ...
    def define_problem(self):
        ...
        for i, comp in enumerate(self.component_dict.keys()):
            ic_expr = Expression(
                conditional(lt(x[1], 25.0), NaOH_amounts[i], HCl_amounts[i]),
                self.comp_func_spaces.sub(i).element.interpolation_points,
            )
            self.set_component_ics(comp, ic_expr)

        ic_expr = Expression(
            conditional(lt(x[1], 25.0), NaOH_amounts[-1], HCl_amounts[-1]),
